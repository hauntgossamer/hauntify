...
def load_music():
        ...
        if (len(songs) == 0 and picked_music == False) or (click_pick_music):
                picked_music = True
...
def back_music():
        ...
        try:
                songlist.selection_clear(0, "end")
                songlist.selection_set(songs.index(current_song) - 2)
                current_song = songs[songlist.curselection()[0]]
                play_music()
        except:
                songlist.selection_clear(0, "end")
...
organize_menu.add_command(label='Select Folder',command=lambda:[load_music(), clicked_pick_music()])
